...
/*
 * Runtime is O(N*N)
 * Idea is to loop through each pair of element and them to a heap
 * if heap size is greater than k, remove elements.
 */
public class KPairswithSmallestSum {
	...
	public List<int[]> kSmallestPairs1(int[] nums1, int[] nums2, int k) {
		Queue<int[]> pq = new PriorityQueue<>((n1, n2) -> (n2[0] + n2[1]) - (n1[0] + n1[1]));
		for (int i = 0; i < nums1.length; i++) {
			for (int j = 0; j < nums2.length; j++) {
				pq.add(new int[] { nums1[i], nums2[j] });
				if (pq.size() > k)
